...
def gerar_grelha(area_coberta, espaco=0.0045):
    """Gera uma grelha quadriculada sobre a área coberta."""
    ...
    lat_range = np.arange(max_lat, min_lat, -espaco)  # De cima para baixo
...
with st.sidebar:
    st.title("Multi Cell View")
    st.markdown("### Configuração Geral")

    mapa_tipo = st.selectbox("Tipo de mapa", ["Padrão", "Satélite", "Híbrido"])
    alcance = st.number_input("Alcance Geral (km)", value=3.0, format="%.1f", step=0.1)
...
# Ajustar o zoom para cobrir toda a área das células
if area_coberta is not None:
    mapa.fit_bounds(area_coberta.bounds)

# Adicionando camada de rótulos no modo Híbrido
