...
# Função para converter índices numéricos em rótulos estilo Excel
def indice_para_rotulo(indice):
    rotulo = ""
    while indice >= 0:
        rotulo = chr(65 + (indice % 26)) + rotulo
        indice = indice // 26 - 1
    return rotulo

# Função para gerar a célula de cobertura
...
def gerar_grelha(area_coberta, espaco):
    ...
    for row_index, lat in enumerate(lat_range[:-1]):  
        for col_index, lon in enumerate(lon_range[:-1]):
            etiqueta = f"{indice_para_rotulo(col_index)}{row_index + 1}"
...
def main():
    ...
    tamanho_quadricula = st.sidebar.slider("Tamanho da Quadrícula (m)", 0, 1000, 500, 50)
    ...
    if mostrar_grelha and area_coberta is not None and tamanho_quadricula > 0:
